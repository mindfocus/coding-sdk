...
var _ MappedNullable = &ProjectMemberMemberRef{}

// ProjectMemberMemberRef 
type ProjectMemberMemberRef struct {
	// 是否管理员
	Assignee NullableBool `json:"Assignee"`
	// 部门ID
	DepartmentId NullableInt64 `json:"DepartmentId"`
	// 部门成员ID
	DepartmentMemberId NullableInt64 `json:"DepartmentMemberId"`
	// 部门名
	DepartmentName NullableString `json:"DepartmentName"`
	// 描述ID
	DescribeId NullableString `json:"DescribeId"`
	// 是否是当前查询部门的直接成员
	Pointer NullableBool `json:"Pointer"`
}
...
func NewProjectMemberMemberRefWithDefaults() *ProjectMemberMemberRef {
	...
	this.Assignee = *NewNullableBool(&assignee)
	var departmentName string = ""
	this.DepartmentName = *NewNullableString(&departmentName)
	var describeId string = ""
	this.DescribeId = *NewNullableString(&describeId)
	var pointer bool = false
	this.Pointer = *NewNullableBool(&pointer)
	return &this
}
